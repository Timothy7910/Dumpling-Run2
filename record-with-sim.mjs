...
const output = path.join(root, 'out', `dango-with-sim-${stamp}.mp4`);
...
try {
  ...
  // 1. Simulate to get results first
  ...
  // 2. Clone the sim panel and overlay it on the stage, hide the rest
  await send('Runtime.evaluate', {
    expression: `
      // Clone sim panel
      const simPanel = document.querySelector('.sim-panel').cloneNode(true);
      simPanel.style.position = 'absolute';
      simPanel.style.top = '16px';
      simPanel.style.left = '16px';
      simPanel.style.zIndex = '99999';
      simPanel.style.width = '240px';
      simPanel.style.zoom = '0.75';
      simPanel.style.boxShadow = '0 10px 25px rgba(0,0,0,0.3)';
      
      const stage = document.querySelector('.stage');
      stage.appendChild(simPanel);
